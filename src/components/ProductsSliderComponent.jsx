import React, { Component } from 'react';

class ProductsSlider extends Component {
    render() {
        return (
            <div className='container similar-slider overflow-hidden py-5'>
                <h2 className='title'>Similar Products</h2>
                <p className='message py-2'>You May Like These Products Also</p>
                <div className='d-flex products-row pb-4'>
                <div className='col-3'>
                        <div className='wraper-p'>
                            <div class="card w-100">
                                <img src="/images/Adicolor_Backpack_Black_H35596_01_standard.jpg" class="card-img-top" alt="..." />
                                <div class="card-body">
                                    <h5 class="card-title">Adidas black t-shirt lorem ipsum dolor sit amet.</h5>
                                    <div>
                                        <div class="price-rate-box">
                                            <div className='row p-0 w-100 m-0'>
                                                <div className='col-8 p-0'>
                                                    <div class="after-sale-price w-100">9999 <span>LE</span></div>

                                                    <div class="pricing-box pb-4 d-flex align-items-center">
                                                        <div class="actuale-price pr-4">9999 <span>LE</span></div>
                                                        <div class="sale-percentage shadow-sm px-2 py-1">30%</div>
                                                    </div>
                                                </div>
                                                <div className='brand col-4'><img src='http://localhost:3000/images/1280px-Adidas_Logo.svg.png' /></div>
                                            </div>
                                            <div class="rating-box d-flex">
                                                <ul class="rating rating-with-review-item d-flex">
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="far fa-star"></i></li>
                                                </ul>
                                                <span class="px-4 rating-num">4.5 of 5.0</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="pick-from py-2">Pick From: <span>Genena Mall</span></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className='col-3'>
                        <div className='wraper-p'>
                            <div class="card w-100">
                                <img src="/images/Adicolor_Classics_Track_Jacket_Black_H35609_21_model.jpg" class="card-img-top" alt="..." />
                                <div class="card-body">
                                    <h5 class="card-title">Adidas black t-shirt lorem ipsum dolor sit amet.</h5>
                                    <div>
                                        <div class="price-rate-box">
                                            <div className='row p-0 w-100 m-0'>
                                                <div className='col-8 p-0'>
                                                    <div class="after-sale-price w-100">9999 <span>LE</span></div>

                                                    <div class="pricing-box pb-4 d-flex align-items-center">
                                                        <div class="actuale-price pr-4">9999 <span>LE</span></div>
                                                        <div class="sale-percentage shadow-sm px-2 py-1">30%</div>
                                                    </div>
                                                </div>
                                                <div className='brand col-4'><img src='http://localhost:3000/images/1280px-Adidas_Logo.svg.png' /></div>
                                            </div>
                                            <div class="rating-box d-flex">
                                                <ul class="rating rating-with-review-item d-flex">
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="far fa-star"></i></li>
                                                </ul>
                                                <span class="px-4 rating-num">4.5 of 5.0</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="pick-from py-2">Pick From: <span>Genena Mall</span></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className='col-3'>
                        <div className='wraper-p'>
                            <div class="card w-100">
                                <img src="/images/Adicolor_Classics_Trefoil_Tee_White_H06644_21_model.jpg" class="card-img-top" alt="..." />
                                <div class="card-body">
                                    <h5 class="card-title">Adidas black t-shirt lorem ipsum dolor sit amet.</h5>
                                    <div>
                                        <div class="price-rate-box">
                                            <div className='row p-0 w-100 m-0'>
                                                <div className='col-8 p-0'>
                                                    <div class="after-sale-price w-100">9999 <span>LE</span></div>

                                                    <div class="pricing-box pb-4 d-flex align-items-center">
                                                        <div class="actuale-price pr-4">9999 <span>LE</span></div>
                                                        <div class="sale-percentage shadow-sm px-2 py-1">30%</div>
                                                    </div>
                                                </div>
                                                <div className='brand col-4'><img src='http://localhost:3000/images/1280px-Adidas_Logo.svg.png' /></div>
                                            </div>
                                            <div class="rating-box d-flex">
                                                <ul class="rating rating-with-review-item d-flex">
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="far fa-star"></i></li>
                                                </ul>
                                                <span class="px-4 rating-num">4.5 of 5.0</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="pick-from py-2">Pick From: <span>Genena Mall</span></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className='col-3'>
                        <div className='wraper-p'>
                            <div class="card w-100">
                            <img src="/images/nike-sportswear-club-fleece-unisex-hoodie-bv2654-010-0.jpg" class="card-img-top" alt="..." />
                                <div class="card-body">
                                    <h5 class="card-title">Adidas black t-shirt lorem ipsum dolor sit amet.</h5>
                                    <div>
                                        <div class="price-rate-box">
                                            <div className='row p-0 w-100 m-0'>
                                                <div className='col-8 p-0'>
                                                    <div class="after-sale-price w-100">9999 <span>LE</span></div>

                                                    <div class="pricing-box pb-4 d-flex align-items-center">
                                                        <div class="actuale-price pr-4">9999 <span>LE</span></div>
                                                        <div class="sale-percentage shadow-sm px-2 py-1">30%</div>
                                                    </div>
                                                </div>
                                                <div className='brand col-4'><img src='http://localhost:3000/images/1280px-Adidas_Logo.svg.png' /></div>
                                            </div>
                                            <div class="rating-box d-flex">
                                                <ul class="rating rating-with-review-item d-flex">
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="far fa-star"></i></li>
                                                </ul>
                                                <span class="px-4 rating-num">4.5 of 5.0</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="pick-from py-2">Pick From: <span>Genena Mall</span></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className='col-3'>
                        <div className='wraper-p'>
                            <div class="card w-100">
                                <img src="/images/Adicolor_Classics_Trefoil_Tee_White_H06644_21_model.jpg" class="card-img-top" alt="..." />
                                <div class="card-body">
                                    <h5 class="card-title">Adidas black t-shirt lorem ipsum dolor sit amet.</h5>
                                    <div>
                                        <div class="price-rate-box">
                                            <div className='row p-0 w-100 m-0'>
                                                <div className='col-8 p-0'>
                                                    <div class="after-sale-price w-100">9999 <span>LE</span></div>

                                                    <div class="pricing-box pb-4 d-flex align-items-center">
                                                        <div class="actuale-price pr-4">9999 <span>LE</span></div>
                                                        <div class="sale-percentage shadow-sm px-2 py-1">30%</div>
                                                    </div>
                                                </div>
                                                <div className='brand col-4'><img src='http://localhost:3000/images/1280px-Adidas_Logo.svg.png' /></div>
                                            </div>
                                            <div class="rating-box d-flex">
                                                <ul class="rating rating-with-review-item d-flex">
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="far fa-star"></i></li>
                                                </ul>
                                                <span class="px-4 rating-num">4.5 of 5.0</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="pick-from py-2">Pick From: <span>Genena Mall</span></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className='col-3'>
                        <div className='wraper-p'>
                            <div class="card w-100">
                                <img src="/images/Adicolor_Classics_Track_Jacket_Black_H35609_21_model.jpg" class="card-img-top" alt="..." />
                                <div class="card-body">
                                    <h5 class="card-title">Adidas black t-shirt lorem ipsum dolor sit amet.</h5>
                                    <div>
                                        <div class="price-rate-box">
                                            <div className='row p-0 w-100 m-0'>
                                                <div className='col-8 p-0'>
                                                    <div class="after-sale-price w-100">9999 <span>LE</span></div>

                                                    <div class="pricing-box pb-4 d-flex align-items-center">
                                                        <div class="actuale-price pr-4">9999 <span>LE</span></div>
                                                        <div class="sale-percentage shadow-sm px-2 py-1">30%</div>
                                                    </div>
                                                </div>
                                                <div className='brand col-4'><img src='http://localhost:3000/images/1280px-Adidas_Logo.svg.png' /></div>
                                            </div>
                                            <div class="rating-box d-flex">
                                                <ul class="rating rating-with-review-item d-flex">
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="far fa-star"></i></li>
                                                </ul>
                                                <span class="px-4 rating-num">4.5 of 5.0</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="pick-from py-2">Pick From: <span>Genena Mall</span></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className='col-3'>
                        <div className='wraper-p'>
                            <div class="card w-100">
                                <img src="/images/nike-sportswear-club-fleece-unisex-hoodie-bv2654-010-0.jpg" class="card-img-top" alt="..." />
                                <div class="card-body">
                                    <h5 class="card-title">Adidas black t-shirt lorem ipsum dolor sit amet.</h5>
                                    <div>
                                        <div class="price-rate-box">
                                            <div className='row p-0 w-100 m-0'>
                                                <div className='col-8 p-0'>
                                                    <div class="after-sale-price w-100">9999 <span>LE</span></div>

                                                    <div class="pricing-box pb-4 d-flex align-items-center">
                                                        <div class="actuale-price pr-4">9999 <span>LE</span></div>
                                                        <div class="sale-percentage shadow-sm px-2 py-1">30%</div>
                                                    </div>
                                                </div>
                                                <div className='brand col-4'><img src='http://localhost:3000/images/1280px-Adidas_Logo.svg.png' /></div>
                                            </div>
                                            <div class="rating-box d-flex">
                                                <ul class="rating rating-with-review-item d-flex">
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="fas fa-star"></i></li>
                                                    <li><i class="far fa-star"></i></li>
                                                </ul>
                                                <span class="px-4 rating-num">4.5 of 5.0</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="pick-from py-2">Pick From: <span>Genena Mall</span></div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        );
    }
}

export default ProductsSlider;